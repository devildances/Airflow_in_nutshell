...
# This is just a fast way to be able to import the python functions from the module process_logs in the scripts folder
sys.path.insert(1, '/usr/local/airflow/dags/scripts')

from process_logs import process_logs_func

'''
we need to create new variable through Airflow UI by open
it via browser using this http://localhost:8080 link then
click Admin on the menu bar then choose Variables and create
...
with DAG(dag_id="template_dag", schedule_interval="@daily", default_args=default_args) as dag:
    ...
    # when we type "macros." means we obtain an access to the predefined macros of Airflow
    t0 = BashOperator(
            task_id="t0",
